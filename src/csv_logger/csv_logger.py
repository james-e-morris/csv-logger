...
class CsvLogger(logging.Logger):
    def __init__(
        ...
        Args:
            ...
            name (string, optional): logging name for logs. if None, filename is used. Defaults to None.
            level (logging level | str | int, optional): logging level for logs. Defaults to logging.INFO.
            fmt (str, optional): output format, accepts parameters 'asctime' 'message' 'levelname' 'name'. Defaults to '%(asctime)s,%(message)s'.
        ...
        logging.Logger.__init__(self, name or filename.rsplit(".", 1)[0], level)
        handler = CsvRotatingFileHandler(
            fmt, datefmt, filename, max_size, max_files, header
        )
